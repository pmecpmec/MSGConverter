# AI-Gebruik Logboek
## MSG-3 → Maximo Project

**Student:** Pedro Eduardo Cardoso
**Opleiding:** Associate Degree Software Developer (ADSD)
**Instelling:** Windesheim
**Project Start:** 4 februari 2026
**Doel:** Dagelijkse tracking van AI-gebruik voor transparantie en reflectie

---

## Instructies

**Hoe te gebruiken:**
1. Vul dit logboek DAGELIJKS in
2. Noteer ELKE keer dat je AI gebruikt
3. Wees specifiek over activiteiten
4. Review elke vrijdag je week
5. Update AI-GEBRUIK.md indien nodig

**Template per sessie:**
```
 [Tijd] | AI-[2/3] | [Activiteit]
↳ Output: [Wat AI genereerde]
↳ Review: [Wat ik aanpaste]
↳ Begrijp? [Ja/Nee] | Test? [Ja/Nee] | Compliant? [Ja/Nee]
```

---

## Week 1 (4 feb - 8 feb 2026)

### Maandag 4 februari 2026

**Projectfase:** Setup & Repository Structuur

#### Sessie 1: Repository Setup
 09:00-11:00 | AI-3 | Complete repository structuur opzetten

**Activiteit:**
- Wat: Repository structuur, code templates, documentatie skeletons
- Hoe: AI genereerde complete folder structuur, ik paste aan voor Windesheim deliverables
- Tool: Cursor AI

**Output:**
- AI genereerde: Complete src/, docs/, tests/ structuur met templates
- Ik heb aangepast: Aangepast aan Windesheim structure, toegevoegd business rules placeholders

**Review:**
- Begrijp ik het? Ja - folder structuur logisch en duidelijk
- Getest? Ja - pytest werkt, imports werken
- Compliant? Ja - Privacy check gedaan, geen credentials

**Deliverable:** Repository structuur - Authenticiteitsverklaring toegevoegd in README

---

#### Sessie 2: Documentatie Templates
 14:00-16:00 | AI-3 | Genereren documentatie templates

**Activiteit:**
- Wat: Markdown templates voor projectdefinitie, PvA, etc.
- Hoe: AI genereerde complete docs structure, ik vulde in met project context
- Tool: Cursor AI

**Output:**
- AI genereerde: Alle docs/ mappen met README's en START-HIER gidsen
- Ik heb aangepast: Toegevoegd Babcock-specifieke context, stakeholders

**Review:**
- Begrijp ik het? Ja - documentatie structuur duidelijk
- Getest? Ja - alle links werken, formatting correct
- Compliant? N/A - geen code

**Deliverable:** Documentatie structuur - Authenticiteitsverklaring toegevoegd

---

**Einde Dag Reflectie:**
- Wat ging goed: Repository setup was snel en professioneel
- Wat kan beter: Meer tijd nemen voor business rules review
- Morgen: Start met projectdefinitie schrijven

---

### Dinsdag 5 februari 2026

**Projectfase:** Projectdefinitie

#### Sessie 1: Context & Probleemstelling
 09:30-11:00 | AI-2 | Brainstormen structuur projectdefinitie

**Activiteit:**
- Wat: Structuur voor context analyse en probleemstelling
- Hoe: AI suggereerde frameworks, ik vulde in met Babcock situatie
- Tool: Cursor AI

**Output:**
- AI genereerde: Templates voor context, stakeholder analyse
- Ik heb aangepast: Alle content zelf geschreven gebaseerd op Babcock situatie

**Review:**
- Begrijp ik het? Ja - eigen analyse
- Getest? N/A - geen code
- Compliant? Ja - geen AI-gegenereerde tekst, alleen structuur

**Deliverable:** Context analyse document (WIP)

---

[... TEMPLATE: Vul hieronder verder in voor elke dag ...]

---

### Woensdag 6 februari 2026

**Projectfase:** [Vul in]

#### Sessie 1: [Titel]
 [Tijd] | AI-[2/3] | [Activiteit]

**Activiteit:**
- Wat: [Beschrijving]
- Hoe: [AI → Ik]
- Tool: [Cursor AI / ChatGPT / etc.]

**Output:**
- AI genereerde: [Beschrijving]
- Ik heb aangepast: [Beschrijving]

**Review:**
- [ ] Begrijp ik het? [Ja/Nee - Toelichting]
- [ ] Getest? [Ja/Nee/N/A]
- [ ] Compliant? [Ja/Nee - Privacy check]

**Deliverable:** [Naam] - Authenticiteitsverklaring: [/]

---

**Einde Dag Reflectie:**
- Wat ging goed:
- Wat kan beter:
- Morgen:

---

### Donderdag 7 februari 2026

[... Herhaal template ...]

---

### Vrijdag 8 februari 2026

[... Herhaal template ...]

---

## Week 1 - Summary

**AI-Gebruik deze week:**
- Totaal sessies: [Aantal]
- Meest gebruikt voor: [Code/Docs/Planning/etc.]
- AIAS niveaus gebruikt:
 - Niveau 2 (Exploratie): [X] sessies
 - Niveau 3 (Samenwerking): [Y] sessies

**Deliverables afgerond:**
- [Deliverable 1] - Niveau [X] - Verklaring toegevoegd
- [Deliverable 2] - Niveau [Y] - Verklaring toegevoegd

**Reflectie:**
- **Voordelen:** [Wat ging goed met AI deze week]
- **Uitdagingen:** [Wat was moeilijk]
- **Leerpunten:** [Wat heb ik geleerd]
- **Volgende week:** [Hoe kan ik AI beter gebruiken]

**Compliance Check:**
- [ ] Alle deliverables hebben authenticiteitsverklaring
- [ ] Geen privacy-gevoelige data gedeeld met AI
- [ ] Alle AI-gegenereerde code gereviewed en begrepen
- [ ] Logboek compleet voor deze week

---

## Week 2 (11 feb - 15 feb 2026)

### Maandag 11 februari 2026

**Projectfase:** [Vul in]

[... Gebruik template hierboven ...]

---

[INSTRUCTIE: Kopieer de week template voor elke nieuwe week]

---

## Week 3 ([Datum range])

[... Template ...]

---

## Week 4 ([Datum range])

[... Template ...]

---

## Maandelijkse Review - Februari 2026

**Periode:** 4 feb - 29 feb 2026

### Overall Statistics
- **Totaal AI-sessies:** [Aantal]
- **AIAS niveau verdeling:**
 - Niveau 2: [X]% (Exploratie)
 - Niveau 3: [Y]% (Samenwerking)

### Most Used For
1. [Activiteit 1] - [Aantal] keer
2. [Activiteit 2] - [Aantal] keer
3. [Activiteit 3] - [Aantal] keer

### Deliverables Met AI
- [Deliverable] - Niveau [X]
- [Deliverable] - Niveau [Y]
[... lijst ...]

### Reflectie Maand
**Grootste voordeel van AI deze maand:**
[Beschrijving]

**Grootste uitdaging:**
[Beschrijving]

**Belangrijkste leerpunt:**
[Beschrijving]

**Verbetering volgende maand:**
[Concrete actie]

### Compliance Status
- [ ] Alle deliverables gedocumenteerd: [XX/XX]
- [ ] Alle verklaringen compleet: [XX/XX]
- [ ] Privacy checks consistent: [/]
- [ ] AI-GEBRUIK.md up-to-date: [/]

---

## Maandelijkse Review - Maart 2026

[... Herhaal template ...]

---

## Project Totaal - Eindreflectie

**Project periode:** 4 februari - [Einddatum] 2026

### Overall AI-Gebruik
- **Totaal sessies project:** [Aantal]
- **Totaal deliverables:** [Aantal]
- **AI-documentatie compleetheid:** [%]

### Impact van AI op Project
**Positieve impact:**
- [Impact 1]
- [Impact 2]

**Uitdagingen:**
- [Uitdaging 1]
- [Uitdaging 2]

**Belangrijkste leerpunten:**
1. [Leerpunt 1]
2. [Leerpunt 2]
3. [Leerpunt 3]

### Aanbevelingen voor Toekomstige Projecten
1. [Aanbeveling 1]
2. [Aanbeveling 2]

### Assessment Voorbereiding
**Kan ik uitleggen:**
- [ ] Waarom ik AI heb gebruikt
- [ ] Hoe ik AI heb gebruikt
- [ ] Welke output ik heb aangepast
- [ ] Wat mijn eigen bijdrage was
- [ ] Welke keuzes ik heb gemaakt

**Belangrijkste talking points:**
1. [Point 1]
2. [Point 2]
3. [Point 3]

---

## Quick Reference

### AIAS Niveaus
- **Niveau 2:** AI voor brainstorming, planning, structuur (geen AI-tekst in output)
- **Niveau 3:** AI als co-auteur, kritisch gereviewed en aangepast

### Activiteit → Niveau Mapping
- Planning/Brainstorming → Niveau 2
- Code development → Niveau 3
- Testing → Niveau 3
- Technisch ontwerp → Niveau 3
- API documentation → Niveau 3
- README files → Niveau 3

### Privacy Checklist (ALTIJD!)
- [ ] Geen echte Maximo credentials
- [ ] Geen echte vliegtuigregistraties
- [ ] Geen stakeholder persoonsgegevens
- [ ] Geen API keys of tokens
- [ ] .env files niet gedeeld

---

## 🆘 Troubleshooting

**Vergeten te loggen?**
→ Reconstrueer zo goed mogelijk, gebruik Git history als hulp

**Te veel detail?**
→ Gebruik afkortingen, focus op essentie

**Te weinig tijd?**
→ Minimum: Tijd, Niveau, Activiteit, Deliverable

**Twijfel over niveau?**
→ Check AI-GEBRUIK.md sectie per projectfase

---

**Laatste update:** 11 februari 2026
**Eigenaar:** Pedro Eduardo Cardoso
**Status:** Actief tracking

---

*Update dit bestand DAGELIJKS! Het is je bescherming én je reflectie-tool.*
